...
if (!GEMINI_API_KEY) {
    console.error("FATAL: GEMINI_API_KEY environment variable not set. Chat API cannot function.");
...
async function getDashboardSummary() {
    try {
        ...
        const negativeCount = await Feedback.countDocuments({ isNegative: true, timestamp: { $gte: oneDayAgo } });
        const recentGuestFeedback = await Feedback.find({ source: 'Guest', rating: { $exists: true }, timestamp: { $gte: oneDayAgo } }).select('rating');
        let averageRating = 'N/A';
        if (recentGuestFeedback.length > 0) {
            const sum = recentGuestFeedback.reduce((acc, curr) => acc + curr.rating, 0);
...
router.post('/message', async (req, res) => {
    ...
    try {
        ...
        if (!apiResponse.ok) {
            ...
            console.error(`Google AI API Error ${apiResponse.status}:`, JSON.stringify(errorBody, null, 2));
            ...
            const detail = errorBody?.error?.message || apiResponse.statusText;
             ...
             if (apiResponse.status === 400) userMessage = "Sorry, there was an issue with the request format.";
            if (apiResponse.status === 429) userMessage = "Sorry, the chat service is busy. Please try again shortly.";
            if (apiResponse.status === 404) userMessage = "Sorry, the requested AI model was not found."; // Could happen if MODEL_NAME is bad
            if (errorBody?.error?.details?.some(d => d.reason === 'SAFETY')) { // Check detailed safety block
                 userMessage = "Sorry, the request or response was blocked for safety reasons.";
            } else if (errorBody?.error?.message?.includes('SAFETY')) { // Check general safety message
                 userMessage = "Sorry, the request or response was blocked for safety reasons.";
            }
        ...
        let botText = '';
         if (responseData?.candidates?.[0]?.content?.parts?.[0]?.text) {
           botText = responseData.candidates[0].content.parts[0].text;
             ...
             console.error("Could not extract text from AI response structure:", JSON.stringify(responseData, null, 2));
        ...
        if (currentHistory.length > MAX_HISTORY_LENGTH * 2) {
            chatHistoryStore[sessionId] = currentHistory.slice(-MAX_HISTORY_LENGTH * 2);
        }
        ...
        res.status(500).json({ error: error.message || "Sorry, I encountered an unexpected error. Please try again later." });
    }
});
