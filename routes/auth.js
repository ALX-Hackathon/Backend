...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (foundUser) {
      ...
      const token = jwt.sign(payload, process.env.JWT_SECRET || "somesecret", {
        expiresIn: "1h"
      });
...
router.post("/refresh", async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: "Server error during refresh token processing." });
  }
});
 ...
 * /api/auth/logout:
 *   post:
 *     summary: Logout and invalidate a refresh token.
 *     tags:
 *       - Auth
 *     requestBody:
 *       required: true
